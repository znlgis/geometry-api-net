...
namespace Esri.Geometry.Core.Operators
{
    ...
    {
        private static readonly Lazy<ClipOperator> _instance = new Lazy<ClipOperator>(() => new ClipOperator());
        ...
        {
            throw new NotImplementedException("Clip operator requires an envelope parameter. Use Execute(geometry, clipEnvelope, spatialRef) instead.");
        }
        ...
        public Geometries.Geometry Execute(Geometries.Geometry geometry, Envelope clipEnvelope, SpatialReference.SpatialReference? spatialRef = null)
        {
            if (geometry == null)
            {
            ...
            {
                throw new ArgumentNullException(nameof(clipEnvelope));
            }

            if (geometry.IsEmpty || clipEnvelope.IsEmpty)
            {
                return CreateEmptyGeometry(geometry.Type);
            }
            ...
            if (!geomEnv.Intersects(clipEnvelope))
            {
                return CreateEmptyGeometry(geometry.Type);
            }
            ...
            {
                return ClipPoint(point, clipEnvelope);
            }

            if (geometry is MultiPoint multiPoint)
            {
                return ClipMultiPoint(multiPoint, clipEnvelope);
            }

            if (geometry is Envelope envelope)
            {
                return ClipEnvelope(envelope, clipEnvelope);
            }

            if (geometry is Line line)
            {
                return ClipLine(line, clipEnvelope);
            }

            if (geometry is Polyline polyline)
            {
                return ClipPolyline(polyline, clipEnvelope);
            }
        ...
        private Point ClipPoint(Point point, Envelope clipEnvelope)
        {
            if (clipEnvelope.Contains(point))
            {
                if (point.Z.HasValue)
                {
                    return new Point(point.X, point.Y, point.Z.Value);
                }
        ...
        {
            double xMin = Math.Max(envelope.XMin, clipEnvelope.XMin);
            double yMin = Math.Max(envelope.YMin, clipEnvelope.YMin);
            double xMax = Math.Min(envelope.XMax, clipEnvelope.XMax);
            double yMax = Math.Min(envelope.YMax, clipEnvelope.YMax);
        ...
        private Geometries.Geometry ClipLine(Line line, Envelope clipEnvelope)
        {
            ...
            if (clipped.Count == 2)
            {
                return new Line(clipped[0], clipped[1]);
            }
