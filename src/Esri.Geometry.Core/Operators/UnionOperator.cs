...
namespace Esri.Geometry.Core.Operators
{
    ...
    {
        private static readonly Lazy<UnionOperator> _instance = new Lazy<UnionOperator>(() => new UnionOperator());
        ...
        public Geometries.Geometry Execute(Geometries.Geometry geometry1, Geometries.Geometry geometry2, SpatialReference.SpatialReference? spatialReference = null)
        {
            ...
            {
                double xMin = Math.Min(env1.XMin, env2.XMin);
                double yMin = Math.Min(env1.YMin, env2.YMin);
                double xMax = Math.Max(env1.XMax, env2.XMax);
                double yMax = Math.Max(env1.YMax, env2.YMax);
            ...
            double minX = Math.Min(envelope1.XMin, envelope2.XMin);
            double minY = Math.Min(envelope1.YMin, envelope2.YMin);
            double maxX = Math.Max(envelope1.XMax, envelope2.XMax);
            double maxY = Math.Max(envelope1.YMax, envelope2.YMax);
