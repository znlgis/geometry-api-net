...
namespace Esri.Geometry.Core.Operators
{
    ...
    {
        private static readonly Lazy<EqualsOperator> _instance = new Lazy<EqualsOperator>(() => new EqualsOperator());
        ...
        public bool Execute(Geometries.Geometry geometry1, Geometries.Geometry geometry2, SpatialReference.SpatialReference? spatialRef = null)
        {
            ...
            {
                return Math.Abs(env1.XMin - env2.XMin) < 1e-10 &&
                       Math.Abs(env1.YMin - env2.YMin) < 1e-10 &&
                       Math.Abs(env1.XMax - env2.XMax) < 1e-10 &&
                       Math.Abs(env1.YMax - env2.YMax) < 1e-10;
            }

            // For other geometry types, this would require more complex implementations
            throw new NotImplementedException($"Equals test between {geometry1.Type} and {geometry2.Type} is not yet implemented.");
        }
    }
}
