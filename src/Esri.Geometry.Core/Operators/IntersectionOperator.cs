...
namespace Esri.Geometry.Core.Operators
{
    ...
    {
        private static readonly Lazy<IntersectionOperator> _instance = new Lazy<IntersectionOperator>(() => new IntersectionOperator());
        ...
        public Geometries.Geometry Execute(Geometries.Geometry geometry1, Geometries.Geometry geometry2, SpatialReference.SpatialReference? spatialReference = null)
        {
            ...
            {
                double xMin = Math.Max(env1.XMin, env2.XMin);
                double yMin = Math.Max(env1.YMin, env2.YMin);
                double xMax = Math.Min(env1.XMax, env2.XMax);
                double yMax = Math.Min(env1.YMax, env2.YMax);
            ...
            if (IntersectsOperator.Instance.Execute(geometry1, geometry2))
            {
                ...
                double xMin = Math.Max(env1_temp.XMin, env2_temp.XMin);
                double yMin = Math.Max(env1_temp.YMin, env2_temp.YMin);
                double xMax = Math.Min(env1_temp.XMax, env2_temp.XMax);
                double yMax = Math.Min(env1_temp.YMax, env2_temp.YMax);
