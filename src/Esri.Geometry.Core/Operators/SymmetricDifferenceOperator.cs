...
namespace Esri.Geometry.Core.Operators
{
    ...
    {
        private static readonly SymmetricDifferenceOperator _instance = new SymmetricDifferenceOperator();

        /// <summary>
        /// Gets the singleton instance of the SymmetricDifferenceOperator.
        /// </summary>
        public static SymmetricDifferenceOperator Instance => _instance;
        ...
        public Geometries.Geometry Execute(Geometries.Geometry geometry1, Geometries.Geometry geometry2, SpatialReference.SpatialReference? spatialReference = null)
        {
            if (geometry1 == null || geometry2 == null)
                throw new System.ArgumentNullException("Geometries cannot be null");
            ...
            if (geometry1 is Point point && geometry2 is MultiPoint mp2)
            {
                ...
                for (int i = 0; i < mp2.Count; i++)
                {
                    var p = mp2.GetPoint(i);
                    if (System.Math.Abs(p.X - point.X) < 1e-10 && System.Math.Abs(p.Y - point.Y) < 1e-10)
                    {
                        pointFound = true;
                    }
                    else
            ...
            if (geometry1 is MultiPoint && geometry2 is Point)
            {
                return Execute(geometry2, geometry1, spatialReference);
            }
            ...
            if (geometry1 is MultiPoint mp1A && geometry2 is MultiPoint mp2A)
            {
                ...
                for (int i = 0; i < mp1A.Count; i++)
                {
                    var p1Test = mp1A.GetPoint(i);
                    bool foundInMp2 = false;

                    for (int j = 0; j < mp2A.Count; j++)
                    {
                        var p2Test = mp2A.GetPoint(j);
                        if (System.Math.Abs(p1Test.X - p2Test.X) < 1e-10 && System.Math.Abs(p1Test.Y - p2Test.Y) < 1e-10)
                        {
                            foundInMp2 = true;
                            break;
                        }
                ...
                {
                    var p2Test = mp2A.GetPoint(j);
                    bool foundInMp1 = false;

                    for (int i = 0; i < mp1A.Count; i++)
                    {
                        var p1Test = mp1A.GetPoint(i);
                        if (System.Math.Abs(p2Test.X - p1Test.X) < 1e-10 && System.Math.Abs(p2Test.Y - p1Test.Y) < 1e-10)
                        {
                            foundInMp1 = true;
                            break;
                        }
