...
namespace Esri.Geometry.Core.Operators
{
    ...
    {
        private static readonly Lazy<GeneralizeOperator> _instance = 
            new Lazy<GeneralizeOperator>(() => new GeneralizeOperator());
        ...
        public Geometries.Geometry Execute(Geometries.Geometry geometry, double maxDeviation, SpatialReference.SpatialReference? spatialReference = null)
        {
            if (geometry == null)
        ...
        private List<Point> GeneralizePath(IReadOnlyList<Point> path, double maxDeviation)
        {
            ...
            while (i < path.Count - 1)
            {
                ...
                for (int j = i + 2; j < path.Count; j++)
                {
                    ...
                    for (int k = i + 1; k < j; k++)
                    {
                        double dist = PerpendicularDistance(path[k], path[i], path[j]);
