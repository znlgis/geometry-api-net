...
namespace Esri.Geometry.Core.IO
{
    ...
    public static class WktImportOperator
    {
        ...
        {
            var match = Regex.Match(wkt, @"POINT\s*Z?\s*\(\s*([\d\.\-\+eE]+)\s+([\d\.\-\+eE]+)(?:\s+([\d\.\-\+eE]+))?\s*\)", RegexOptions.IgnoreCase);
            if (!match.Success)
            {
                throw new FormatException($"Invalid POINT WKT: {wkt}");
            }
        ...
        private static Polyline ParseLineString(string wkt)
        {
            ...
            if (!match.Success)
            {
                throw new FormatException($"Invalid LINESTRING WKT: {wkt}");
            }
        ...
        private static Polygon ParsePolygon(string wkt)
        {
            ...
            if (!match.Success)
            {
                throw new FormatException($"Invalid POLYGON WKT: {wkt}");
            }
        ...
        private static MultiPoint ParseMultiPoint(string wkt)
        {
            ...
            if (!match.Success)
            {
                throw new FormatException($"Invalid MULTIPOINT WKT: {wkt}");
            }
        ...
        private static Polyline ParseMultiLineString(string wkt)
        {
            ...
            if (!match.Success)
            {
                throw new FormatException($"Invalid MULTILINESTRING WKT: {wkt}");
            }
        ...
        private static List<List<Point>> ParseRings(string text)
        {
            ...
            var depth = 0;
            var currentRing = new System.Text.StringBuilder();
        ...
        private static List<Point> ParseCoordinateList(string text)
        {
            ...
            foreach (var pair in coordPairs)
            {
                ...
                if (coords.Length < 2)
                {
                    throw new FormatException($"Invalid coordinate pair: {pair}");
                }
