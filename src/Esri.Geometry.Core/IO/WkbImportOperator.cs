...
public static class WkbImportOperator
{
  ...
  {
    if (wkb == null || wkb.Length == 0) throw new ArgumentException("WKB data cannot be null or empty.", nameof(wkb));
  ...
  {
    var numPoints = ReadInt32(reader, bigEndian);
  ...
  {
    var numRings = ReadInt32(reader, bigEndian);
  ...
  private static MultiPoint ReadMultiPoint(BinaryReader reader, bool bigEndian)
  {
    ...
    for (var i = 0; i < numPoints; i++)
    {
      ...
      if (pointType != WKB_POINT) throw new FormatException($"Expected point type in multipoint, got {pointType}");
  ...
  {
    var numLineStrings = ReadInt32(reader, bigEndian);
    var polyline = new Polyline();
    ...
    for (var i = 0; i < numLineStrings; i++)
    {
      ...
      var numPoints = ReadInt32(reader, lsBigEndian);
      var points = new List<Point>(numPoints);
