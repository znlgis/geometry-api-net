...
namespace Esri.Geometry.Core.IO
{
    ...
    public class GeoJsonImportOperator
    {
        ...
        private static Geometries.Geometry ParseGeometry(JsonElement element)
        {
            ...
            string type = typeElement.GetString() ?? throw new ArgumentException("GeoJSON type cannot be null");

            if (!element.TryGetProperty("coordinates", out JsonElement coordinatesElement))
            {
