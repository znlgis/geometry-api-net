...
{
  private static readonly Lazy<EsriJsonExportOperator> _instance = new(() => new EsriJsonExportOperator());
  ...
  private string ExportPoint(Point point)
  {
    ...
    if (point.Z.HasValue) sb.Append($",\"z\":{FormatNumber(point.Z.Value)}");
    if (point.M.HasValue) sb.Append($",\"m\":{FormatNumber(point.M.Value)}");
  ...
  private string ExportMultiPoint(MultiPoint multiPoint)
  {
    ...
    for (var i = 0; i < multiPoint.Count; i++)
    {
      ...
      var point = multiPoint.GetPoint(i);
